...
def test_single_chains(a):
    assert np.all(a.belongs_up_chain | a.belongs_down_chain | a.belongs_couple)


@pytest.fixture
def a_with_triples(a):
...
def test_show_triples(a_with_triples, items):
    ...
    if request.config.getoption("--do_display"):
        ...
        disp_array[3, :] = a.is_triple_begin.astype(int)
        ...
        row_labels = ['Values', 'Lo Is Better', 'Belongs to Couple', 'Is Triple begin']
