...
def insert_best(square_extension_tile, square_storage_location_tile, upper_left_idx, square_storage_location_map, square_extension_map):
    while True:
        found, persistent, best_idx_tile = assign_best(square_extension_tile, square_storage_location_tile)
        if found:
...
def image_squares_select(square_storage_location_map, square_extension_map, num_tiles_expand_noshift_shift, shift1, shift2):
    sz_tile=2*sz_halftile
    num_tiles_expand_row = num_tiles_expand_noshift_shift[shift1[0]][0]
    num_tiles_expand_col = num_tiles_expand_noshift_shift[shift1[1]][1]
    for I in range(0, num_tiles_expand_row):
        for J in range(0, num_tiles_expand_col):
            i=shift1[0]*sz_halftile + sz_tile * I
            j=shift1[1]*sz_halftile + sz_tile * J
            nshift = (1 - shift1[0], 1 - shift1[1])
            upper_left_idx = (i+sz_halftile*nshift[0], j+sz_halftile*nshift[1])

            if upper_left_idx[0]+sz_tile >= square_extension_map.shape[0] or upper_left_idx[1]+sz_tile >= square_extension_map.shape[1]:
               continue
    ...
    num_tiles_expand_row = num_tiles_expand_noshift_shift[shift2[0]][0]
    num_tiles_expand_col = num_tiles_expand_noshift_shift[shift2[1]][1]
    for I in range(0, num_tiles_expand_row):
        for J in range(0, num_tiles_expand_col):
            i = shift1[0] * sz_halftile + sz_tile * I
            j = shift1[1] * sz_halftile + sz_tile * J
            ...
            if upper_left_idx[0] + sz_tile >= square_extension_map.shape[0] or upper_left_idx[1] + sz_tile >= \
                    square_extension_map.shape[1]:
                continue

            square_extension_tile = np.array(square_extension_map[upper_left_idx[0]:upper_left_idx[0] + sz_tile,
                                             upper_left_idx[1]:upper_left_idx[1] + sz_tile])
            square_storage_location_tile = np.array(
                square_storage_location_map[upper_left_idx[0]:upper_left_idx[0] + sz_tile - 1,
                upper_left_idx[1]:upper_left_idx[1] + sz_tile - 1])
            success, square_storage_location_map, square_extension_map = insert_best(square_extension_tile,
                                                                                     square_storage_location_tile,
                                                                                     upper_left_idx,
