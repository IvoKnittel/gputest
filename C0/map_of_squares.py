...
class SquareItem:
    ...
    conflicts:     graph_ids of other graphs that exclude this item's graph - i.e.
                    choosing a member of this graph would block a member of theirs.
                    Populated by do_closure once every item in the graph has a
                    graph_id; empty until then.
    centrality:    distance, in .link hops, from this item's chain to the closest
                    terminal (no-.link) item of its graph; assigned by
                    closure.find_central_graph_items one generation at a time, -1
                    while unassigned.
    """
    quality: float = -1.0
